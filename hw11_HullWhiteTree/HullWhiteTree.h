...
private:
    ...
    int jmax;
    vector<vector<double>> tree;      // interest rate tree before calibration
    vector<vector<double>> r_tudi;    // process of r_tudi
    vector<double> gamma;             // gamma
    vector<double> mt;                // m_delta
    vector<double> bond_p;            // true interest rate over 30 years
    vector<vector<double>> rt;        // interest rate after calibration
    
public:
    HullWhiteTree();                         // constructor
    ~HullWhiteTree();                        // deconstructor
    void get_r_tudi();
    void get_gamma();
    void get_mt();
    void get_rt();
